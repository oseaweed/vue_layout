  ...
  <div id="app">
    ...
    <mu-row class="main-content">
      ...
      <mu-col
        ...
        :tablet="width.components"
        :desktop="width.components"
      >
...
export default {
  ...
  computed: {
    ...
    width: {
      //三栏的宽度设置
      get() {
        return _Vue.$store.state.width[0];
      }
    },
    parentComponent: {
      get() {
        ...
        if (this.current.parentId)
          component = _Vue.$store.state.components.find(
  ...
  methods: {
    ...
    oprate(e) {
      // let components = await _Vue.$store.dispatch('delComponent', this.current.info.id)
    ...
    openUiDocument() {
      switch (this.current.info.ui) {
        ...
        case "Mint-UI":
          return window.open(
            "https://mint-ui.github.io/docs/#/zh-cn/" +
              this.current.info.name.replace(" ", "-")
          );
        case "iView-UI":
          return window.open(
            "https://www.iviewui.com/components/" + this.current.info.name
          );
        default:
    ...
    createShare() {
      ...
      share.set("store", _Vue.$store.state);
      share.save().then(res => {
        this.share.url =
