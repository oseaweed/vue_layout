<template>
  <section class="preview" @dragover="dragOver" @drop="drop">
    ...
    <mu-paper class="preview-head">
      <div class="bar">
        ...
        >
          ...
          <mu-menu-item
            :title="previewMode==='pc'?'手机模式':'PC模式'"
    ...
    <mu-content-block class="preview-tip" v-if="components.length===0">试试拖拽组件进来</mu-content-block>
    <!-- 组件拖动属于嵌套操作时，这个弹出框会弹出 -->
    <mu-popover
      v-if="current.info"
      :trigger="popover.trigger"
      :open="popover.open"
      @close="popover.open=false"
    >
      <mu-menu @change="selectedSlot">
        <label>&nbsp;&nbsp;嵌套到{{current.info.name}}:</label>
        <mu-menu-item
...
export default {
  ...
  methods: {
    ...
    drop(e) {
      ...
      if (
        e.target.className.indexOf("sound-code") !== -1 ||
        e.target.className.indexOf("hljs") !== -1
      )
        return;

      let isNest =
        e.target.className.indexOf("preview") === -1 &&
      ...
      if (isNest) {
        ...
        selectSlot.then(slot => {
          ...
          let index = components.findIndex(
            item => item.info.id === this.current.info.id
          );
          //嵌套模板
          ...
          topComponent = getTemplate(
            topComponent.info,
            topComponent.attributes,
            topComponent.slots
          );
          let topIndex = components.findIndex(
            c => c.info.id === topComponent.info.id
          );
        ...
        if (!this.insertPosition.position) {
          ...
          let index = components.findIndex(
            component =>
              component.info.id === this.insertPosition.component.info.id
          );
    ...
    clickPreview(e) {
      ...
      if (componentHTML) {
        ...
        let component = this.components.find(
          component => component.info.id === currentId
        );
        if (component)
    ...
    getSource(components) {
      ...
      code += `\n</section></template>`;
      //添加用户编辑的css
      let cssText = _Vue.$store.state.css;
      if (cssText) {
        cssText = "\n<style scoped>\n" + cssText;
        cssText += "\n</style>\n";
        code += cssText;
    ...
    getParentComponent(component) {
      ...
      if (component.parentId) {
        return this.getParentComponent(
          components.find(c => c.info.id === component.parentId)
        );
      } else {
        return component;
      }
    ...
    del: async function() {
      let components = await _Vue.$store.dispatch(
        "delComponent",
    ...
    undo() {
      let components = JSON.parse(
        JSON.stringify(_Vue.$store.state.backupComponents)
      );
      _Vue.$store.commit("setState", {
        backupComponents: [],
        components
    ...
    copyComponent() {
      ...
      let getCopiedComponents = (component = current) => {
        let hasChild = Object.keys(component.slots).filter(
          slot => component.slots[slot].length
        ).length;
    ...
    pasteComponent() {
      ...
      selectSlot.then(slot => {
        let copiedComponents = JSON.parse(
          JSON.stringify(_Vue.$store.state.copiedComponents)
        );
