...
var handle = function (_attr, _slots, { id }) {
  ...
  if (attributes.itemQuantity.value < 0) attributes.itemQuantity.value = 0;

  //根据组件不同需要做的不同操作
  let max = _attr.itemQuantity
    ? _attr.itemQuantity.value
    : attributes.itemQuantity.value;
  ...
  template = template.replace(
    `:itemQuantity="${attributes.itemQuantity.value}"`,
    ""
  );
