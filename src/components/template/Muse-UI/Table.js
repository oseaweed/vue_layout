...
var handle = function (_attr, _slots, { id }) {
  ...
  if (attributes.column.value < 0) attributes.column.value = 0;
  if (attributes.row.value < 0) attributes.row.value = 0;
  ...
  for (let i = 0; i < column; i++) {
    if (headerSlots[i]) {
      ...
      let thId = guid();
      //添加要嵌入到th中的text组件
      let text = addComponent(
        { name: "Text", ui: "Common", id: guid() },
        thId,
        { text: { type: "text", value: "表头" + i } }
      );
      let thSlots = { default: [{ id: text.info.id }] };
      //添加th组件
      let thComponent = addComponent(
        { name: "Th", ui: "Muse-UI", id: thId },
        header.info.id,
        {},
        thSlots
      );
  ...
  let addColumn = function (column, parentId) {
    ...
    for (let j = 0; j < column; j++) {
      ...
      let tdComponent = addComponent(
        { name: "Td", ui: "Muse-UI", id: tdId },
        parentId,
        {},
        tdSlots
      );
  ...
  for (let i = 0; i < row; i++) {
    if (body.slots.default[i]) {
      let index = components.findIndex(
        (c) => c.info.id === body.slots.default[i].id
      );
      let len = components[index].slots.default.length;
      //行未改变，列增加的情况
      if (len < column) {
        let td = addColumn(column - len, components[index].info.id);
        components[index].slots.default.push.apply(
          components[index].slots.default,
          td
        );
      }
      //行未改变，列减少了的情况
      ...
      let trComponent = addComponent(
        { name: "Tr", ui: "Muse-UI", id: trId },
        body.info.id,
        {},
        { default: td }
      );
  ...
  function del(slots) {
    Object.keys(slots).forEach((slot) => {
      slots[slot].forEach((slot) => {
        ...
        if (Object.keys(child.slots)) del(child.slots);
      });
    });
  }
  //column数值减少时，子属性要对应的删除
  function reduce(_slots, _slot) {
    let arr = _slots.filter((item) => {
      //得到所有要删除的对象
  ...
  template = template.replace(
    `:column="${attributes.column.value}"\n:row="${attributes.row.value}"`,
    ""
  );
