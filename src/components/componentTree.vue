  ...
  <section>
    <div v-for="c in components">
      <a @click="click(c.info.id)">
        <mu-icon
          @click="show=!show"
          :value="show?'keyboard_arrow_down':'keyboard_arrow_right'"
      ...
      <div v-for="(slot,key) in c.slots" v-show="show">
        <componentTree
          :components="slotToComponents(c.slots[key])"
...
export default {
  ...
  methods: {
    slotToComponents(slot) {
      return _Vue.$store.state.components.filter(
        c => slot.findIndex(item => item.id === c.info.id) !== -1
      );
    },
    click(id) {
